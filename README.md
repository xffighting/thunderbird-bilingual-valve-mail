...
- 打开英文或俄文邮件时，自动识别语言并在原文下一行显示中文译文。
- 每行译文旁提供“纠错”，记录后在设置页人工批准，才会成为长期术语。
...
- 俄文业务句自动走“俄语 → 英语 → 中文”本机离线模型链路，英文仍走英译中模型。
- 增加 26 条俄语阀门与外贸术语规则，包括球阀、阀体、阀体材质、交期和报价单。
- 纠错保存成功后弹窗自动关闭并显示“✓”；保存失败时保留弹窗，可直接重试。
- 同一原文的待审核纠错只保留一条；设置页批准后，按英语或俄语方向加入长期术语。
...
- 在用户目录创建独立 Python 环境。
- 下载英文到中文、俄文到英文、中文到英文、英文到俄语和英文到阿拉伯语 Argos Translate 模型。
...
6. 选择本目录中的 `manifest.json`；正式安装可选择 `dist/game-mail-summary-v0.7.1.xpi`。
...
- 译文旁点击“纠错”，填写英文或俄文术语/短语与正确中文；保存成功会自动关闭弹窗，记录进入设置页等待批准。
