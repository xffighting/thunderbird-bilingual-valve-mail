...
def main() -> None:
    with tempfile.TemporaryDirectory() as temporary:
        ...
        manifest, payload = release("1.3.0")
        ...
        assert result["currentVersion"] == "1.3.0", result
        assert requests == [
            {
        ...
        assert provider.load_active_bundle()["datasetVersion"] == "1.3.0"
        rollback = provider.rollback()
        assert_uniform(rollback)
        assert rollback["state"] == "ROLLED_BACK", rollback
        assert rollback["currentVersion"] == "1.2.0", rollback
    ...
    for label, change in (
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            assert result["currentVersion"] == "1.2.0", (label, result)

    with tempfile.TemporaryDirectory() as temporary:
        manifest, payload = release("1.3.0", corrupt=True)
        provider, _ = provider_for(pathlib.Path(temporary), payload)
        result = provider.apply(manifest)
        assert not result["ok"] and result["error"]["code"] == "INVALID_BUNDLE", result

    with tempfile.TemporaryDirectory() as temporary:
        manifest, payload = release("1.3.0", schema="2.0.0")
        provider, _ = provider_for(pathlib.Path(temporary), payload)
        result = provider.apply(manifest)
    ...
    for failure in (OSError("offline"), TimeoutError("timeout")):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            result = provider.apply(manifest)
            assert not result["ok"] and result["currentVersion"] == "1.2.0", result
    ...
    with tempfile.TemporaryDirectory() as temporary:
        ...
        (root / "versions/1.3.0/bundle.json").write_text(
            "{corrupted",
            encoding="utf-8",
        )
        recovered = provider.load_active_bundle()
        assert recovered["datasetVersion"] == "1.2.0", recovered["datasetVersion"]
        status = provider.status()
        assert status["state"] == "ROLLED_BACK", status

    with tempfile.TemporaryDirectory() as temporary:
        root = pathlib.Path(temporary)
        first_manifest, first_payload = release("1.3.0")
        ...
        assert downgrade["currentVersion"] == "1.3.0"
    ...
    with tempfile.TemporaryDirectory() as temporary:
        ...
        assert kept == ["1.4.0", "1.5.0", "1.6.0"], kept
